...
public class KickCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            GuiMenu menu = new GuiMenu("Kick: " + name);

            menu.set(0, new ItemStackBuilder(Material.ARROW).withName("&bGo Back").build());
            menu.bindLeft(0, "punish " + name);

            menu.set(1, 3, new ItemStackBuilder(Material.BOOK).withName("&6Use Kick Preset").build());
            menu.bindLeft(12, "kick " + name + " preset");

            menu.set(1, 5, new ItemStackBuilder(Material.NAME_TAG).withName("&6Write Custom Kick").build());
            menu.bindLeft(14, "kick " + name + " custom");
            ...
            if (function.equalsIgnoreCase("preset")) {
                GuiMenu menu = new GuiMenu("Select a Kick Preset", 4);
                ...
                for (PunishmentPreset preset : PunishmentPreset.values()) {
                    if (preset.getPunishmentType().equals(Punishment.PunishmentType.KICK)) {
                        ItemStack icon = new ItemStackBuilder(Material.INK_SACK)
                                .withName("&c" + preset.getReason()).withData(1);
                        menu.add(icon);
                        menu.bindLeft(icon, "kick " + name + " preset " + preset.getId());
                    }
                }
            ...
            //if (!Arrays.asList(bungeeUtil.getOnlinePlayers()).contains(target.getName())) {
            //    player.sendMessage(ChatUtil.format("&cYou cannot kick that player because they are not online."));
            //    return false;
            //}
            BPlayer targetB = BlockcadeUsers.getBPlayer(target.getUniqueId());
            ...
            if (function.equalsIgnoreCase("preset")) {
                try {
                    ...
                    Main.getBungeeUtil().kickPlayer(player, targetB.getName(), ChatUtil.format(punishment.getReason()));
                    Main.getBungeeUtil().messageStaff(player, "Punish", sender + " &7kicked " +
                    targetB.getFormattedName() + " &7for &b" + punishment.getReason());
